...
from General_functions import SimpleDataset, permute_input_tensor, Normalize

seed = 100
gene_name_ls = ['CHRNB3'] #{'CHRNA3', 'CHRNA5', 'CHRNA6', 'CHRNB3', 'CHRNB4'}
stt_file = './res/022425/imp_score_ukb_y1001.csv'
pval_file = './res/022425/pval_imp_ukb_y1001.csv'
...
#L2_lambda_range = torch.FloatTensor([0.001, 0.01, 0.05, 0.1, 0.5, 1., 5.])
L2_lambda_range = torch.FloatTensor([0.])
lambda_best_file = './res/022425/L2_lambda_best_ukb_1001.csv'
...
def main():
    for gene_name in gene_name_ls:
        ...
        ## Transfering on race
        #pheno_data = pheno_data[pheno_data[:,3] == '1001',:]
        ...
        #Gxp_tensor = torch.FloatTensor(gene_data.values[:,1]).reshape([gene_data.shape[0], 1])
        ...
        returnscore = []
        returnscorelist = []
        #importance_score_all = np.zeros((k_folds))
        #importance_score_list_all = np.zeros((k_folds))
        ...
        for fold, (train_ids, valid_ids) in enumerate(kfold.split(simple_dataset)):
            ...
            for l2_lambda in L2_lambda_range:
                ...
                torch.save({
                    ...
                    }, './Parameters/nn_checkpoints_'+gene_name+'_ukb_y1001_'+str(lambda_idx)+'.pth')
            ...
            checkpoint = torch.load('./Parameters/nn_checkpoints_'+gene_name+'_ukb_y1001_'+str(l2_lambda_best_idx)+'.pth')
            DNN_best.load_state_dict(checkpoint['model_state_dict'])
            
            torch.save({
                    'model_state_dict': DNN_best.state_dict(),
                    }, './Parameters/nn_checkpoints_'+gene_name+'_ukb_y1001.pth')
            ...
            for X_test, y_test in valid_loader:
                ...
                for epoch in range(permute_epoch):
                    inputs_perm = permute_input_tensor(inputs_test, cov_idx = list(range(X.shape[-1])))
                ...
                #score_k = np.sum(pred_loss - pred_loss_perm_avg)/inputs_test.shape[0]
                score_k = pred_loss - pred_loss_perm_avg
                scorelist_k = pred_loss - pred_loss_perm
                #emp_var_i = np.var(pred_loss - pred_loss_perm, axis = 1)
        ...
        stt_np = np.array([gene_name, stt, stt_std])
        ...
        #standard_stt = np.abs(stt/np.sqrt(stt_var))
        #rv = NormalDist(mu = 0., sigma = 1.)
        #pval = 1-rv.cdf(standard_stt)
        #print(standard_stt)
        #print(pval)
        standard_stt = stt/stt_std
        rv = NormalDist(mu = 0., sigma = 1.)
        pval = rv.cdf(standard_stt)
        pval_np = np.append(np.array([gene_name]), pval)
