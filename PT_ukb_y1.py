...
stt_file = './res/022425/imp_score_ukb_y1.csv'
pval_file = './res/022425/pval_imp_ukb_y1.csv'
...
lambda_best_file = './res/022425/L2_lambda_best_ukb_y1.csv'
...
def main():
    for gene_name in gene_name_ls:
        ...
        pheno_data = pd.read_csv('./ukb/Y_1.csv')
        ## Transfering on race
        ...
        for fold, (train_ids, valid_ids) in enumerate(kfold.split(simple_dataset)):
            ...
            for l2_lambda in L2_lambda_range:
                ...
                torch.save({
                    ...
                    }, './Parameters/nn_checkpoints_'+gene_name+'_ukb_y1_'+str(lambda_idx)+'.pth')
            ...
            checkpoint = torch.load('./Parameters/nn_checkpoints_'+gene_name+'_ukb_y1_'+str(l2_lambda_best_idx)+'.pth')
            DNN_best.load_state_dict(checkpoint['model_state_dict'])
            
            torch.save({
                    'model_state_dict': DNN_best.state_dict(),
                    }, './Parameters/nn_checkpoints_'+gene_name+'_ukb_y1.pth')
