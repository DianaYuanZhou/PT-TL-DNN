...
seed = 100
gene_name_ls = ['CHRNA3'] #{'CHRNA3', 'CHRNA5', 'CHRNA6', 'CHRNB3', 'CHRNB4'}
stt_file = './res/031625/imp_score_2hl_tl_ukb_y_1002_gp.csv'
pval_file = './res/031625/pval_imp_2hl_tl_ukb_y_1002_gp.csv'
### Simulating Dataset
k_folds = 3
...
L2_lambda_range = torch.FloatTensor([0., 0.001, 0.01, 0.05, 0.1, 0.5, 1., 5., 10.])
lambda_best_file = './res/031625/L2_lambda_best_y_1002_gp.csv'
...
def main():
    ...
    for gene_name in gene_name_ls:
        ...
        pheno_data = pd.read_csv('./ukb/Y_1002.csv')
        ## Transfering on race
        ...
        for fold, (train_ids, valid_ids) in enumerate(kfold.split(simple_dataset)):
            ...
            for l2_lambda in L2_lambda_range:
                ...
                DNN = DeepNeuralNetwork(input_size = X.shape[-1], hidden_size1 = hidden_size1, hidden_size2 = hidden_size2, output_size = 1)
                checkpoint0 = torch.load('./Parameters/nn_checkpoints_'+gene_name+'_ukb_y1001.pth')
                DNN.load_state_dict(checkpoint0['model_state_dict'])
                
                ### Freeze all parameters expect for last FC layer
                for param in DNN.parameters():
                    param.requires_grad = False
                DNN.last_fc = nn.Linear(hidden_size2, 1)
                ...
                for X_train, y_train in train_loader:
                    inputs = X_train.double()
                    labels = y_train.double().reshape(-1, 1)
                ...
                torch.save({
                    ...
                    }, './Parameters/nn_checkpoints_'+gene_name+'_TL_ukb_y1002_'+str(lambda_idx)+'.pth')
            ...
            checkpoint = torch.load('./Parameters/nn_checkpoints_'+gene_name+'_TL_ukb_y1002_'+str(l2_lambda_best_idx)+'.pth')
            DNN_best.load_state_dict(checkpoint['model_state_dict'])
            
            torch.save({
                    'model_state_dict': DNN_best.state_dict(),
                    }, './Parameters/nn_checkpoints_'+gene_name+'_TL_ukb_y1002.pth')
