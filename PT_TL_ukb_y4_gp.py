...
stt_file = './res/031625/imp_score_2hl_tl_ukb_y4_gp.csv'
pval_file = './res/031625/pval_imp_2hl_tl_ukb_y4_gp.csv'
### Simulating Dataset
k_folds = 3
...
lambda_best_file = './res/031625/L2_lambda_best_y4_gp.csv'
...
def main():
    ...
    for gene_name in gene_name_ls:
        ...
        pheno_data = pd.read_csv('./ukb/Y_4.csv')
        ## Transfering on race
        ...
        for fold, (train_ids, valid_ids) in enumerate(kfold.split(simple_dataset)):
            ...
            for l2_lambda in L2_lambda_range:
                ...
                torch.save({
                    ...
                    }, './Parameters/nn_checkpoints_'+gene_name+'_TL_ukb_y4_'+str(lambda_idx)+'.pth')
            ...
            checkpoint = torch.load('./Parameters/nn_checkpoints_'+gene_name+'_TL_ukb_y4_'+str(l2_lambda_best_idx)+'.pth')
            DNN_best.load_state_dict(checkpoint['model_state_dict'])
            
            torch.save({
                    'model_state_dict': DNN_best.state_dict(),
                    }, './Parameters/nn_checkpoints_'+gene_name+'_TL_ukb_y4.pth')
